...
│   ├── index.md            # slim master: lists domains only (generated)
│   ├── index/              # per-domain sub-indexes (generated)
│   │   └── <domain>.md     # page list for one domain
│   ├── log.md              # chronological record (append-only, top entries newest)
│   ├── hot.md              # hot cache: ~500-word context summary (overwritten)
│   ├── overview.md         # optional executive summary of the entire wiki
│   ├── concepts/<domain>/  # ideas, patterns, frameworks
│   ├── entities/<domain>/  # people, orgs, products, repos
│   ├── sources/<domain>/   # one summary page per raw source
│   ├── domains/            # domain hub pages — wiki/domains/<slug>.md
│   ├── comparisons/<domain>/   # side-by-side analyses
│   ├── questions/<domain>/     # filed answers to user queries
...
Every page lives under **`wiki/<type-folder>/<domain>/<slug>.md`**:

- Type folder is the plural of the frontmatter `type`: `concepts/`, `entities/`, `sources/`, `comparisons/`, `questions/`, `meta/`. Domain hub pages are the exception — they live at `wiki/domains/<slug>.md` (the domain itself).
- `<domain>` is a kebab slug from frontmatter `domain`. Pages without a domain go under `_global/`.
- `<slug>` is the kebab title.
- Single-segment writes at `wiki/` root are rejected. Only allowed at root: `index.md`, `log.md`, `hot.md`, `overview.md`, `README.md`.

### Rules

- `.raw/` is read-only. Never modify source files.
- Every wiki page has frontmatter — minimum: `type`, `title`, `domain`, `status`, `created`, `updated`, `tags`.
- Wikilinks over paths. Use `[[stem]]` (filename without path or extension); stems must be unique vault-wide.
- Atomic notes. One concept per page. If it covers two things, split it.
- Update, don't duplicate. If a page exists, update it.
- Don't hand-edit `wiki/index.md` or `wiki/index/*.md` — they're regenerated by `kb_reindex` from a frontmatter scan.
...
domain: <kebab-slug>      # use _global if genuinely cross-cutting
created: 2026-04-07
updated: 2026-04-07
tags:
  - <domain-tag>
...
`domain` drives both the on-disk path and the sub-index it lands in. Pick a stable, broad slug per knowledge area (`clearance`, `ai-research`, `accounting`). Splitting domains later is cheap; renaming them isn't.
...
**concept**: `complexity` (basic|intermediate|advanced), `aliases` (list)
...
`wiki/index.md` and `wiki/index/<domain>.md` are **generated** by the `kb_reindex` tool from a frontmatter scan. Do not hand-edit them — your edits will be overwritten on the next reindex. Run `kb_reindex` after every ingest, save, or page move.

### wiki/index.md (slim master)

Lists domains only. Always loaded into context — keep it tiny regardless of vault size.
...
- [[index/clearance|clearance]] — 12 page(s)
- [[index/finance|finance]] — 5 page(s)
- [[index/_global|_global]] — 3 page(s)
```

### wiki/index/<domain>.md (per-domain sub-index)

One file per domain, listing its pages grouped by type. Loaded only when the model is working on that domain.
...
Generated by `kb_reindex`. 12 page(s).

## Concepts
- [[risk-rating]] — How risk levels are computed

## Entities
- [[james-officer]] — Senior reviewer

## Sources
- [[2024-clearance-handbook]] — Official handbook excerpt
```
...
2. Read wiki/index.md — slim domain map
3. For a specific domain, read wiki/index/<domain>.md
4. Use kb_query to find pages — don't browse folders
5. Read 3–5 pages max per question

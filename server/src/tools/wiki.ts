...
export const wikiTools: Tool[] = [
  {
    name: "wiki_ingest",
    description:
      "Read a source document from the vault's .raw/ folder. Returns full content for the model to extract entities, concepts, and write structured wiki pages. Does NOT itself write — the model decides what pages to author and uses vault_write.",
    inputSchema: {
      type: "object",
      properties: {
        vault: { type: "string" },
  ...
  },
  {
    name: "wiki_query",
    description:
      "Search the wiki/ folder for a query. Returns matched pages with snippets so the model can synthesize an answer.",
    inputSchema: {
      type: "object",
      properties: {
        vault: { type: "string" },
...
async function ingest(cfg: VaultConfig, args: Record<string, unknown>) {
  ...
  const target = vaultPath(vault, path.join(".raw", source));
